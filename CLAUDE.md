...
| CONSULTANT (EXTERNAL — PT Mitra Pajak Sentosa) | ERP Staff | external.consultant@mitrapajak.com | TestPassword123! |
...
> **product-identity 결정 ① (2026-07-24)**: `CONSULTANT`/`TAX_ADVISOR` = EXTERNAL 세무법인 전용. JTC 신고 실무는 `TAX_OPERATOR` 계열. 옛 `consultant.test@jakartatax.co.id` 는 폐기됐고, `advisor.test@jakartatax.co.id` 는 consultant 은퇴 후 `TAX_OPERATOR_SUPERVISOR` 로 존속(supervisor.test 와 동급). 신규 JTC consultant INSERT 는 `forbid_jtc_consultant` 트리거가 차단. ERP(consultant) 테스트는 `external.consultant`, JTC 실무 테스트는 operator 계정으로 수행. 상세: `docs/01-plan/features/jtc-consultant-to-operator-migration.md`.

Seed scripts:
- `npm run db:seed-test-users` — 개인/법인 customer + admin (JTC consultant 생성 없음 — 결정 ①; EXTERNAL·operator 는 seed-master-and-external.ts)
...
- `SEED_TARGET=prod npx tsx scripts/test-id-billing-exception.ts` — 승인 없이 예외 발행 계약 (수정요청 #26, 워크큐 트리거): customer 403 + 사유 누락/단문 400 + 예외발행 200(APPROVED·작성본 게이트 우회, 미발행·세액 게이트 유지) + `is_exception`/`issue_reason` 각인 + 큐 EBILLING_GENERATED 전이(승인 없이) + 중복 404, 9 asserts, `[IDBILL-EXC-E2E]` sentinel
...
- `SEED_TARGET=prod npx tsx scripts/test-workqueue-employees.ts` — 직원 인사 기록 워크큐 계약 (PPh21 큐 anchor + 직원 마스터 flags green/red + 무NPWP/PTKP/급여0 검출 + 변경이력 + RBAC 403, 14 asserts, `[WQ-EMP-E2E]` sentinel, period 2099-11)
- `SEED_TARGET=prod npx tsx scripts/test-workqueue-visibility.ts` — 워크큐 가시성 계약 (wht-import → PPh23 큐 자동생성 + 미배정 항목 상담원 GET 노출 + 첫 액션 시 자동 claim, 9 asserts, `[WQVIS-E2E]` sentinel, period 2027-01)
