...
- `SEED_TARGET=prod npx tsx scripts/validate-pph21-e2e.ts` — PPh21 employee import parser → auto-map → CSV → multipart POST → DB → cleanup (BINTANG JAYA real xlsx, 5 rows + `[SMOKE-PPH21-E2E]` sentinel prefix; auto-skip if file absent)
