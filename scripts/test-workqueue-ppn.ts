/**
 * Workqueue PPN smoke test:
 *   quick-create(PPN) → GET ppn shape → request PENDING_DOCS(+400) → RBAC 403 → cleanup.
 * Run with: SEED_TARGET=prod npx tsx scripts/test-workqueue-ppn.ts
 * Sentinel period 2099-12. Sentinel prefix: [WQ-PPN-E2E].
 */
...
async function main() {
  console.log('🧾 Workqueue PPN smoke test\n');
  ...
  await admin.from('djp_submission_queue').delete()
    .eq('customer_id', customer.id).eq('tax_type', 'PPN')
  ...
  const cr = await api(operatorToken!, 'POST', '/api/operator/queue', {
    customerId: customer.id, taxType: 'PPN', month: SENTINEL_MONTH, year: SENTINEL_YEAR,
  });
  ...
  console.log('\n━━ 2. GET ppn shape ━━');
  const detail = await api(operatorToken!, 'GET', `/api/operator/workqueue/${qid}/ppn`);
  console.log(`   ${detail.status}`);
  assert(detail.json.success === true, 'ppn detail success');
  const d = detail.json.data as { summary?: { fakturCount?: unknown }; rows?: unknown } | undefined;
  assert(typeof d?.summary?.fakturCount === 'number', 'summary.fakturCount is number');
  assert(Array.isArray(d?.rows), 'rows is array');

  console.log('\n━━ 3. request → PENDING_DOCS ━━');
  const rq = await api(operatorToken!, 'POST', `/api/operator/workqueue/${qid}/request`, { message: '[WQ-PPN-E2E] Coretax 대조 요청' });
  ...
  if (customerToken) {
    const forbidden = await api(customerToken, 'GET', `/api/operator/workqueue/${qid}/ppn`);
  ...
  console.log('\n✅ PASS — workqueue PPN contract verified.');
}
