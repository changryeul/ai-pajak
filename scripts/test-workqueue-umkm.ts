/**
 * Workqueue UMKM/PPh Final smoke test:
 *   quick-create(PPh_FINAL) → GET umkm shape → request PENDING_DOCS(+400) → RBAC 403 → cleanup.
 * Run with: SEED_TARGET=prod npx tsx scripts/test-workqueue-umkm.ts
 * Sentinel period 2099-12. Sentinel prefix: [WQ-UMKM-E2E].
 */
...
async function main() {
  console.log('🧾 Workqueue UMKM/PPh Final smoke test\n');
  ...
  await admin.from('djp_submission_queue').delete()
    .eq('customer_id', customer.id).eq('tax_type', 'PPh_FINAL')
  ...
  console.log('\n━━ 1. quick-create PPh_FINAL ━━');
  const cr = await api(operatorToken!, 'POST', '/api/operator/queue', {
    customerId: customer.id, taxType: 'PPh_FINAL', month: SENTINEL_MONTH, year: SENTINEL_YEAR,
  });
  ...
  console.log('\n━━ 2. GET umkm shape ━━');
  const detail = await api(operatorToken!, 'GET', `/api/operator/workqueue/${qid}/umkm`);
  console.log(`   ${detail.status}`);
  assert(detail.json.success === true, 'umkm detail success');
  const d = detail.json.data as { summary?: { recordCount?: unknown }; rows?: unknown } | undefined;
  assert(typeof d?.summary?.recordCount === 'number', 'summary.recordCount is number');
  assert(Array.isArray(d?.rows), 'rows is array');

  console.log('\n━━ 3. request → PENDING_DOCS ━━');
  const rq = await api(operatorToken!, 'POST', `/api/operator/workqueue/${qid}/request`, { message: '[WQ-UMKM-E2E] 선납분 납부 확인 요청' });
  ...
  if (customerToken) {
    const forbidden = await api(customerToken, 'GET', `/api/operator/workqueue/${qid}/umkm`);
  ...
  console.log('\n✅ PASS — workqueue UMKM contract verified.');
}
