...
function validateFakturFormat(num: string): { valid: boolean; reason: string } {
  ...
  if (cleaned.length !== 16) {
    return { valid: false, reason: `16자리 필요 (현재 ${cleaned.length}자리)` };
  }

  if (!/^\d{16}$/.test(cleaned)) {
    return { valid: false, reason: '숫자만 입력 가능' };
  }

  // Transaction code (first 2 digits): 01-09
  const txCode = parseInt(cleaned.substring(0, 2));
  if (txCode < 1 || txCode > 9) {
    return { valid: false, reason: `거래 코드 ${txCode} 유효하지 않음 (01-09)` };
  }

  // Status code (3rd digit): 0 or 1
  const statusCode = parseInt(cleaned.substring(2, 3));
  if (statusCode !== 0 && statusCode !== 1) {
    return { valid: false, reason: `상태 코드 ${statusCode} 유효하지 않음 (0: 일반, 1: 대체)` };
  }

  // Year code (digits 4-5)
  const yearCode = parseInt(cleaned.substring(3, 5));
  if (yearCode < 10 || yearCode > 30) {
    return { valid: false, reason: `연도 코드 ${yearCode} 범위 밖 (10-30)` };
  }

  return { valid: true, reason: '형식 정상' };
}
  ...
  const verify = async () => {
    ...
    for (const num of numbers) {
      ...
      newResults.push({
        ...
        details: !format.valid ? format.reason : isDuplicate ? '중복 번호 감지' : '정상',
      });
    }
    ...
    <div className="container mx-auto py-8 px-4 max-w-3xl">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-800 via-emerald-700 to-teal-900 p-6 md:p-8 text-white mb-6">
        ...
        <div className="relative z-10">
          <p className="text-green-300 text-sm flex items-center gap-2">
            <Shield className="h-4 w-4" />e-Faktur Validator
          </p>
          <h1 className="text-2xl md:text-3xl font-bold mt-1">e-Faktur PPN 검증</h1>
          <p className="text-green-300 mt-2 text-sm">Faktur Pajak 번호의 형식, 유효성, 중복 여부를 확인합니다</p>
        </div>
      </div>
      ...
      <Card className="border-0 shadow-sm mb-6">
        <CardContent className="p-5 space-y-3">
          <Label className="text-sm font-medium">Faktur Pajak 번호 입력</Label>
          {numbers.map((num, i) => (
          ...
          <div className="flex justify-between">
            <Button variant="outline" size="sm" onClick={addRow}>
              <Plus className="h-3.5 w-3.5 mr-1" />번호 추가
            </Button>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Card className="border-0 shadow-sm border-l-4 border-l-green-500">
              <CardContent className="p-3 text-center">
                <p className="text-xs text-green-600 flex items-center justify-center gap-1"><CheckCircle className="h-3 w-3" />정상</p>
                <p className="font-bold text-lg text-green-700">{validCount}</p>
              </CardContent>
            </Card>
            <Card className="border-0 shadow-sm border-l-4 border-l-red-500">
              <CardContent className="p-3 text-center">
                <p className="text-xs text-red-600 flex items-center justify-center gap-1"><XCircle className="h-3 w-3" />오류</p>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="p-4">
              <h3 className="font-medium text-sm mb-2 flex items-center gap-2">
                <FileText className="h-4 w-4 text-gray-400" />Faktur Pajak 번호 형식 가이드
              </h3>
