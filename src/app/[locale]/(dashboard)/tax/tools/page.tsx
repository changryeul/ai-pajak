...
export default function TaxToolsPage() {
  ...
  const tools = [
    { id: 'predict' as const, icon: TrendingUp, title: 'Prediksi Pajak', desc: 'Simulasi pajak berdasarkan penghasilan', gradient: 'from-blue-500 to-indigo-600' },
    { id: 'cashflow' as const, icon: BarChart3, title: 'Prediksi Arus Kas', desc: 'Forecast arus kas + cadangan pajak', gradient: 'from-green-500 to-emerald-600' },
    { id: 'summarize' as const, icon: FileText, title: 'Ringkas Dokumen', desc: 'AI ringkas regulasi pajak', gradient: 'from-purple-500 to-violet-600' },
    { id: 'customs' as const, icon: DollarSign, title: 'Kalkulator Bea Masuk', desc: 'Hitung pajak impor', gradient: 'from-amber-500 to-orange-500' },
    { id: 'regional' as const, icon: MapPin, title: 'Pajak Daerah', desc: 'PBB, BPHTB, Restoran, Hotel', gradient: 'from-red-500 to-rose-500' },
    { id: 'benchmark' as const, icon: Building2, title: 'Benchmarking', desc: 'Bandingkan dengan industri', gradient: 'from-cyan-500 to-blue-500' },
  ];
  ...
  async function runTool(tool: ActiveTool, body: Record<string, unknown>, endpoint: string) {
    ...
    } catch { setResult({ error: 'Gagal memproses' }); }
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 mb-6">
        {tools.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => { setActiveTool(isActive ? null : t.id); setResult(null); }}
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg bg-gradient-to-br ${t.gradient} shadow-sm`}>
    ...
    <Card className="border-0 shadow-sm">
      <CardContent className="p-5 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div><Label className="text-xs">Gaji Bulanan (Rp)</Label><Input type="number" value={income} onChange={e => setIncome(e.target.value)} placeholder="15000000" /></div>
        ...
        <Button onClick={() => onSubmit({ currentMonthlyIncome: Number(income), ptkpStatus: ptkp, taxYear: 2025 })} disabled={isLoading || !income} className="w-full">
          {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <TrendingUp className="h-4 w-4 mr-2" />}Prediksi
        </Button>
      </CardContent>
    </Card>
  );
...
function CashflowForm({ onSubmit, isLoading }: { onSubmit: (b: Record<string, unknown>) => void; isLoading: boolean }) {
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardContent className="p-5 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div><Label className="text-xs">Revenue Bulanan (Rp)</Label><Input type="number" value={revenue} onChange={e => setRevenue(e.target.value)} placeholder="100000000" /></div>
          <div><Label className="text-xs">Pengeluaran Bulanan (Rp)</Label><Input type="number" value={expenses} onChange={e => setExpenses(e.target.value)} placeholder="60000000" /></div>
        </div>
        <Button onClick={() => onSubmit({ monthlyRevenue: Number(revenue), monthlyExpenses: Number(expenses), months: 6 })} disabled={isLoading || !revenue} className="w-full">
          {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <BarChart3 className="h-4 w-4 mr-2" />}Forecast
        </Button>
      </CardContent>
    </Card>
  );
...
function SummarizeForm({ isLoading, setIsLoading, setResult }: { isLoading: boolean; setIsLoading: (v: boolean) => void; setResult: (v: unknown) => void }) {
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      setResult(data.success ? data.data : { error: data.error });
    } catch { setResult({ error: 'Upload gagal' }); }
    finally { setIsLoading(false); }
  };
  return (
    <Card className="border-0 shadow-sm">
      <CardContent className="p-5 space-y-3">
        <Label className="text-xs">Upload dokumen regulasi (PDF/Image)</Label>
        <input type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={handleUpload} disabled={isLoading} className="text-sm" />
        {isLoading && <div className="flex items-center gap-2 text-sm text-gray-500"><Loader2 className="h-4 w-4 animate-spin" />AI sedang meringkas...</div>}
      </CardContent>
    </Card>
  );
}
    ...
    <Card className="border-0 shadow-sm">
      <CardContent className="p-5 space-y-3">
        <div className="grid grid-cols-3 gap-3">
          <div><Label className="text-xs">CIF (USD)</Label><Input type="number" value={cif} onChange={e => setCif(e.target.value)} placeholder="1000" /></div>
          <div><Label className="text-xs">Kurs (IDR/USD)</Label><Input type="number" value={rate} onChange={e => setRate(e.target.value)} /></div>
          <div><Label className="text-xs">Bea Masuk (%)</Label><Input type="number" value={duty} onChange={e => setDuty(e.target.value)} step="0.01" /></div>
        </div>
        <Button onClick={() => onSubmit({ cifValue: Number(cif), exchangeRate: Number(rate), dutyRate: Number(duty) })} disabled={isLoading || !cif} className="w-full">
          {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <DollarSign className="h-4 w-4 mr-2" />}Hitung
        </Button>
      </CardContent>
    </Card>
  );
...
function RegionalForm({ onSubmit, isLoading }: { onSubmit: (b: Record<string, unknown>) => void; isLoading: boolean }) {
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardContent className="p-5 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div><Label className="text-xs">Jenis Pajak</Label>
        ...
        <Button onClick={() => {
          ...
          {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <MapPin className="h-4 w-4 mr-2" />}Hitung
        </Button>
      </CardContent>
    </Card>
  );
...
function BenchmarkForm({ onSubmit, isLoading }: { onSubmit: (params: string) => void; isLoading: boolean }) {
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardContent className="p-5 space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <div><Label className="text-xs">Industri</Label>
          ...
          <div><Label className="text-xs">Tarif Efektif Anda (%)</Label>
        ...
        <Button onClick={() => onSubmit(`industry=${industry}&effectiveRate=${rate}`)} disabled={isLoading} className="w-full">
          {isLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Building2 className="h-4 w-4 mr-2" />}Bandingkan
        </Button>
      </CardContent>
    </Card>
  );
