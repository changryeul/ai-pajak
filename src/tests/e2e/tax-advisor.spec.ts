...
test.describe('TAX_ADVISOR_JTC Role Tests', () => {
  ...
  test('❌ Tax Advisor CANNOT file tax with POA SCOPE mismatch', async ({ request }) => {
    const response = await request.post('/api/tax/file', {
      headers: createAuthHeaders(advisorToken),
      data: {
        customerId: TEST_SCOPE_MISMATCH_CUSTOMER.customerId,
  ...
  test('✅ Tax Advisor can sign POA for their tax partner', async ({ request }) => {
    // Use PENDING_SIGNATURE POA (customer has already signed)
    const response = await request.post('/api/poa/sign', {
      headers: createAuthHeaders(advisorToken),
      data: {
        poaId: TEST_POA_PENDING.id,
  ...
  test('❌ Tax Advisor CANNOT sign POA for DIFFERENT tax partner', async ({ request }) => {
    // Using a non-existent POA ID to simulate different tax partner's POA
    // In production, this would return 403 for a POA from another tax partner
    // But since the POA doesn't exist, we get 404
    const response = await request.post('/api/poa/sign', {
      headers: createAuthHeaders(advisorToken),
      data: {
        poaId: '00000000-0000-0000-0000-000000009999',
        signatureData: 'signature',
        signatureMethod: 'DIGITAL',
      },
    });

    // POA not found returns 404
    expect(response.status()).toBe(404);

    const body = await response.json();
    expect(body.error).toBe('POA not found');
  });

  test('❌ Tax Advisor CANNOT sign POA before customer signs', async ({ request }) => {
    // Use UNSIGNED DRAFT POA (customer has not signed yet)
    const response = await request.post('/api/poa/sign', {
      headers: createAuthHeaders(advisorToken),
      data: {
        poaId: TEST_POA_UNSIGNED_DRAFT.id,
        signatureData: 'signature',
        signatureMethod: 'DIGITAL',
      },
    });
  ...
  test('✅ Tax filing includes complete metadata', async ({ request }) => {
    ...
    // Verify tax details
    expect(body.tax.taxType).toBe(taxFiling.taxType);
    expect(body.tax.taxPeriod).toBe(taxFiling.taxPeriod);
    expect(body.tax.taxYear).toBe(taxFiling.taxYear);
    expect(body.tax.netTaxDue).toBe(taxFiling.taxData.netTaxDue);
